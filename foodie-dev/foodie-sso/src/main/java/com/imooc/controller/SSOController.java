...
public class SSOController {
    ...
    public String login(String returnUrl,
        ...
        // 1. 获取userTicket全局门票，如果cookie中能够获取到，证明用户登录过，此时签发一个一次性的临时票据并且回跳

        String cookie_user_ticket = getCookie(request, COOKIE_USER_TICKET);

        // 1.1 全局门票验证， 如果验证通过则创建临时门票，"redirect:" + returnUrl + "?tmpTicket=" + tmpTicket;
        // cookie_user_ticket -》 REDIS_USER_TICKET：usetocket => userid => REDIS_USER_TOKEN:user
        // 而临时票据都不相关，就是临时生成，临时销毁
        boolean verified = verifyUserTicket(cookie_user_ticket);
    ...
    public String doLogin(String username,
        ...
        // 3.1 用户全局门票需要放入CAS端的cookie中--在浏览器中呈现--sso.com
    ...
    public IMOOCJSONResult verifyTmpTicket(String tmpTicket,
        ...
        // 1. 验证并且获取用户的userTicket--全局门票--通过全局门票了来获取用户id
