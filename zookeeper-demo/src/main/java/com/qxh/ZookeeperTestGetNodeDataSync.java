...
public class ZookeeperTestGetNodeDataSync implements Watcher {
    ...
    public static void main(String[] args) throws IOException, InterruptedException, KeeperException {
        ...
        zooKeeper.create(PREFIX_SYNC, "hellodata".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE,
    ...
    public void process(WatchedEvent event) {
        if(Event.KeeperState.SyncConnected == event.getState()){
            if(Event.EventType.None == event.getType() && null == event.getPath()){
                ...
                try {
                    System.out.println("the data of:" +  event.getPath() + " is::" +
                            new String(zooKeeper.getData(event.getPath(), true, stat)));
                    System.out.println("watch czxid::"+stat.getCzxid()+",mzxid::" + stat.getMzxid() +
